      ...
      <div className={cn('relative mx-auto aspect-[5/4] max-w-sm', compact ? 'w-24' : 'w-full')}>
        ...
        <svg viewBox="0 0 220 180" className="relative h-full w-full" aria-label={`${pose.name} animated pose`}>
          ...
          >
            ...
            >
              <circle cx={shape.head[0]} cy={shape.head[1]} r="12" fill="#f4c7a1" stroke="#125a47" strokeWidth="3" />
              <circle cx={shape.head[0] + 4} cy={shape.head[1] - 2} r="1.5" fill="#125a47" />
              <path d={`M${shape.head[0] + 4} ${shape.head[1] + 4} q3 2 5 0`} fill="none" stroke="#125a47" strokeWidth="1.5" strokeLinecap="round" />
            </motion.g>
            <motion.path
              d={shape.torso}
              ...
              {shape.arms.map((arm) => {
                ...
                return (
                  <React.Fragment key={arm}>
                    <motion.path d={arm} animate={isResting ? { d: arm } : { d: [arm, shiftPath(arm, movementShift.arms), arm] }} transition={{ duration: isResting ? 3 : motionDuration, repeat: Infinity, ease: 'easeInOut' }} fill="none" stroke="#075b4a" strokeWidth="14" strokeLinecap="round" strokeLinejoin="round" />
                    <motion.path d={arm} animate={isResting ? { d: arm } : { d: [arm, shiftPath(arm, movementShift.arms), arm] }} transition={{ duration: isResting ? 3 : motionDuration, repeat: Infinity, ease: 'easeInOut' }} fill="none" stroke="#20c997" strokeWidth="9" strokeLinecap="round" strokeLinejoin="round" />
                    <circle cx={handX} cy={handY} r="5" fill="#f4c7a1" stroke="#075b4a" strokeWidth="2" />
                  </React.Fragment>
                );
              })}
            </motion.g>
              ...
              {shape.legs.map((leg) => {
                ...
                return (
                  <React.Fragment key={leg}>
                    <motion.path d={leg} animate={isResting ? { d: leg } : { d: [leg, shiftPath(leg, movementShift.legs), leg] }} transition={{ duration: isResting ? 3 : motionDuration, repeat: Infinity, ease: 'easeInOut' }} fill="none" stroke="#064b42" strokeWidth="15" strokeLinecap="round" strokeLinejoin="round" />
                    <motion.path d={leg} animate={isResting ? { d: leg } : { d: [leg, shiftPath(leg, movementShift.legs), leg] }} transition={{ duration: isResting ? 3 : motionDuration, repeat: Infinity, ease: 'easeInOut' }} fill="none" stroke="#0f9476" strokeWidth="10" strokeLinecap="round" strokeLinejoin="round" />
                    <ellipse cx={footX} cy={footY} rx="8" ry="4" fill="#f4c7a1" stroke="#064b42" strokeWidth="2" />
                  </React.Fragment>
                );
              })}
            </motion.g>
      ...
      {!compact && (
        <div className="relative text-center">
          <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-700 dark:text-emerald-300">
            {isResting ? 'Transition' : 'Follow the moving shape'}
          </span>
